-- Strengthen office payment idempotency: jsonb result (replayed/paymentId),
-- insert payment before shipment update, handle unique races safely.
-- Historical rows with null client_payment_id remain valid.
-- client_payment_id is unique globally (not only per org) so the same intention
-- key cannot be silently reused under another tenant.
...
begin
  ...
  if coalesce(dup_count, 0) > 0 then
    raise exception
      'Cannot apply idx_shipment_payments_client_payment_id_global: % duplicate client_payment_id value(s) exist',
      dup_count;
  end if;
end;
$$;

create unique index if not exists idx_shipment_payments_client_payment_id_global
  on public.shipment_payments (client_payment_id)
...
begin
  ...
  if safe_operation_id is not null then
    ...
    from public.shipment_payments
    where client_payment_id = safe_operation_id
    limit 1;

    if existing_payment.id is not null then
      if existing_payment.organization_id is distinct from caller_org
         or existing_payment.shipment_id is distinct from target_shipment_id
  ...
  if safe_operation_id is not null then
    ...
    if existing_payment.id is not null then
      if existing_payment.organization_id is distinct from caller_org
         or existing_payment.shipment_id is distinct from shipment_row.id
         or round(existing_payment.amount::numeric, 2) is distinct from round(payment_amount::numeric, 2)
         or existing_payment.method is distinct from safe_method
         or existing_payment.kind is distinct from safe_kind then
        raise exception 'PAYMENT_IDEMPOTENCY_CONFLICT';
...
comment on function public.collect_shipment_invoice_payment(
  ...
  'Authoritative invoice collect. Same client_payment_id + payload = replayed jsonb; org/shipment/amount/method/kind mismatch = PAYMENT_IDEMPOTENCY_CONFLICT. Payment insert precedes shipment update.';
