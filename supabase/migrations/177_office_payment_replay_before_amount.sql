-- Allow identical replay when balance is already zero (lost full-collect response).
-- Resolve client_payment_id before requiring payment_amount > 0 so the action can
-- call the security-definer RPC even if RLS hides shipment_payments SELECT.
...
comment on function public.collect_shipment_invoice_payment(
  ...
  'Authoritative invoice collect. Same client_payment_id + payload = replayed jsonb; org/shipment/amount/method mismatch (kind excluded as derived) = PAYMENT_IDEMPOTENCY_CONFLICT. Payment insert precedes shipment update. Replay resolves before amount>0 so lost full-collect responses work under RLS.';
