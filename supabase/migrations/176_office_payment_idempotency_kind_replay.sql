-- Replay-safe payload fingerprint: exclude derived payment kind (deposit/balance),
-- which can change after a committed abono crosses depositRequired.
-- Intention identity is org + client_payment_id + shipment + amount + method.
...
comment on function public.collect_shipment_invoice_payment(
  ...
  'Authoritative invoice collect. Same client_payment_id + payload = replayed jsonb; org/shipment/amount/method mismatch (kind excluded as derived) = PAYMENT_IDEMPOTENCY_CONFLICT. Payment insert precedes shipment update.';
