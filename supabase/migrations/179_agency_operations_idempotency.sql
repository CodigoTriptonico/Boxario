-- AGE-001: authoritative idempotency for agency request create + assign.
-- Extends idempotency_operations with request_hash (fingerprint), hardens both
-- RPCs, and adds uniqueness so a request line cannot appear in two visits.
-- Does not rewrite historical migrations 083/094/171.

-- ---------------------------------------------------------------------------
-- Fingerprint column (nullable for historical rows; new ops always set it)
-- ---------------------------------------------------------------------------

alter table public.idempotency_operations
  add column if not exists request_hash text;

comment on column public.idempotency_operations.request_hash is
  'Canonical payload fingerprint for conflict detection. Same key + different hash => conflict.';

create index if not exists idempotency_operations_type_key_idx
  on public.idempotency_operations (tenant_id, operation_type, idempotency_key);

-- ---------------------------------------------------------------------------
-- One visit line per request line (prevents duplicate visits/stops for a request)
-- ---------------------------------------------------------------------------
...
begin
  ...
  from (
    ...
    from public.agency_visit_lines
  ...
  if dup_count > 0 then
    raise exception
      'AGE-001: % request_line_id values appear in multiple agency_visit_lines; resolve duplicates before applying unique index',
      dup_count;
  end if;
end;
$$;

create unique index if not exists agency_visit_lines_request_line_uidx
  on public.agency_visit_lines (request_line_id);

comment on index public.agency_visit_lines_request_line_uidx is
  'Invariant: each agency request line belongs to at most one visit (no duplicate assign).';

-- ---------------------------------------------------------------------------
-- create_agency_service_request — client key + fingerprint + secure path
-- ---------------------------------------------------------------------------
